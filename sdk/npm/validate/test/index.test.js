...
test("composeFiles merges compatible declarations", () => {
  ...
  const tempDir = fs.mkdtempSync(
    path.join(os.tmpdir(), "ampersand-validate-test-"),
  );
...
test("composeFiles detects conflicting bindings", () => {
  ...
  assert.ok(
    composed.errors.some(
      (e) => e.includes("conflicting_binding") || e.includes("&memory.graph"),
    ),
  );
});
...
test("checkPipeline validates named pipeline from declaration", () => {
  ...
  const checked = checkPipeline(document, null, {
    pipelineName: "incident_triage",
  });
...
test("generateMcpConfig resolves auto providers from registry where possible", () => {
  ...
  const unresolvedProviders = generated.value.unresolved_providers.map(
    (u) => u.provider,
  );
