defmodule AmpersandCore.Runtime do
    ...
    steps =
      ...
      |> Enum.map(fn {%{capability: capability, operation: operation}, index} ->
        ...
        %{
          ...
          "provider_resolution" => provider_resolution(provider, registry_provider)
        }
        |> drop_nil_values()
      end)
  ...
  defp execute_plan(plan, input, opts) do
    ...
    {step_results, final_output, provenance_records, _last_output_hash} =
      plan["steps"]
      |> Enum.reduce({[], normalize_input(input, plan["source"]), [], nil}, fn step,
                                                                               {results_acc, current_input,
                                                                                provenance_acc, parent_hash} ->
        context = %{
          plan: plan,
        ...
        {
          results_acc ++ [step_result],
          output,
          append_if_present(provenance_acc, provenance_record),
          output_hash
        }
      end)
  ...
  defp provider_resolution("auto", _registry_provider) do
    %{
      "status" => "pending",
